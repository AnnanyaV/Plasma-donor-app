...
def check(email):
    url = "https://f0afnp8is1.execute-api.us-east-1.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://f0afnp8is1.execute-api.us-east-1.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://f0afnp8is1.execute-api.us-east-1.amazonaws.com/plasma/getbloodgroupdata"
    response = requests.get(url)
    r = response.json()
    print(r)
    return render_template('stats.html',b=str(sum(r)),b1=str(r[0]),b2=str(r[1]),b3=str(r[2]),b4=str(r[3]),b5=str(r[4]),b6=str(r[5]),b7=str(r[6]),b8=str(r[7]))
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://f0afnp8is1.execute-api.us-east-1.amazonaws.com/plasma/getbloodgroupdata?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    emailids=[]
    for i in a:
